...
namespace gpmr
{
  ...
  MapReduceJob::MapReduceJob(int & argc, char **& argv)
    : binner(NULL), combiner(NULL), mapper(NULL), partitioner(NULL), partialReducer(NULL), sorter(NULL), reducer(NULL), commRank(-1), commSize(-1), deviceNum(-1)
  {
    int provided;
