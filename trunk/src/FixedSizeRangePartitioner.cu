...
__global__ void gpmrFixedSizeRangePartitionerZeroCountsAndIndicesKernel(int * gpuBucketCounts, int * gpuBucketIndices)
{
  if (blockIdx.x == 0)  gpuBucketCounts [threadIdx.x] = 0;
  else                  gpuBucketIndices[threadIdx.x] = 0;
}
void gpmrFixedSizeRangePartitionerZeroCountsAndIndices(const int commSize, int * gpuBucketCounts, int * gpuBucketIndices, cudaStream_t * stream)
{
  dim3 blockSize(commSize, 1, 1);
  dim3 gridSize(2, 1, 1);
  gpmrFixedSizeRangePartitionerZeroCountsAndIndicesKernel<<<gridSize, blockSize, 0, *stream>>>(gpuBucketCounts, gpuBucketIndices);
}

__global__ void gpmrFixedSizeRangePartitionerCountBucketKernel(const int commSize, const int numKeys, const int bucket, int * gpuKeys, int * gpuBucketCount)
{
...
void gpmrFixedSizeRangePartitionerCount(const int commSize, const int numKeys, int * gpuKeys, int * gpuBucketCounts, cudaStream_t * stream)
{
  dim3 blockSize(512, 1, 1);
  dim3 gridSize((numKeys + 511) / 512, 1, 1);
  for (int i = 0; i < commSize; ++i)
  {
    gpmrFixedSizeRangePartitionerCountBucketKernel<<<gridSize, blockSize, 0, *stream>>>(commSize, numKeys, i, gpuKeys, gpuBucketCounts + i);
  }
}

__global__ void gpmrFixedSizeRangePartitionerPartitionToTempKernel(const int commSize, const int bucket, const int numKeys, int * gpuKeys, int * gpuVals, int * gpuTempKeySpace, int * gpuTempValSpace, int * gpuKeyOffsets, int * gpuBucketIndices)
{
...
void gpmrFixedSizeRangePartitionerPartitionToTemp(const int commSize, const int numKeys, int * gpuKeys, int * gpuVals, int * gpuTempKeySpace, int * gpuTempValSpace, int * gpuKeyOffsets, int * gpuBucketIndices, cudaStream_t * stream)
{
  dim3 blockSize(NUM_THREADS_IN_PARTITION, 1, 1);
  dim3 gridSize(1024, 1, 1);
  for (int i = 0; i < commSize; ++i)
  {
    gpmrFixedSizeRangePartitionerPartitionToTempKernel<<<gridSize, blockSize, 0, *stream>>>(commSize, i, numKeys, gpuKeys, gpuVals, gpuTempKeySpace, gpuTempValSpace, gpuKeyOffsets, gpuBucketIndices);
  }
}

__global__ void gpmrFixedSizeRangePartitionerMoveFromTempKernel(const int numKeys, int * gpuKeys, int * gpuVals, int * gpuTempKeySpace, int * gpuTempValSpace)
{
...
void gpmrFixedSizeRangePartitionerMoveFromTemp(const int commSize, const int numKeys, int * gpuKeys, int * gpuVals, int * gpuTempKeySpace, int * gpuTempValSpace, cudaStream_t * stream)
{
  dim3 blockSize(512, 1, 1);
  dim3 gridSize((numKeys + 511) / 512, 1, 1);
  gpmrFixedSizeRangePartitionerMoveFromTempKernel<<<gridSize, blockSize, 0, *stream>>>(numKeys, gpuKeys, gpuVals, gpuTempKeySpace, gpuTempValSpace);
}

__global__ void gpmrFixedSizeRangePartitionerSetKeyAndValCountsKernel(const int * const keyCounts, int * const valCounts)
{
  valCounts[threadIdx.x] = keyCounts[threadIdx.x];
}
void gpmrFixedSizeRangePartitionerSetKeyAndValCounts(const int commSize, int * gpuKeyCounts, int * gpuValCounts, cudaStream_t * stream)
{
  dim3 blockSize(commSize, 1, 1);
  dim3 gridSize(1, 1, 1);
  gpmrFixedSizeRangePartitionerSetKeyAndValCountsKernel<<<gridSize, blockSize, 0, *stream>>>(gpuKeyCounts, gpuValCounts);
}

__global__ void gpmrFixedSizeRangePartitionerSetKeyAndValOffsetsKernel(const int commSize, const int * const gpuKeyCounts, int * const gpuKeyOffsets, int * const gpuValOffsets)
{
...
void gpmrFixedSizeRangePartitionerSetKeyAndValOffsets(const int commSize, const int * const gpuKeyCounts, int * const gpuKeyOffsets, int * const gpuValOffsets, cudaStream_t * stream)
{
  dim3 blockSize(1, 1, 1);
  dim3 gridSize(1, 1, 1);
  gpmrFixedSizeRangePartitionerSetKeyAndValOffsetsKernel<<<gridSize, blockSize, 0, *stream>>>(commSize, gpuKeyCounts, gpuKeyOffsets, gpuValOffsets);
}
